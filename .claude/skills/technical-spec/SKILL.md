...
Read the source of truth before asking: all of `docs/business/`, plus any existing `CLAUDE.md`, README, or partial specs. The business docs define _what_ is built; this document defines _how_. Then interview the user one question at a time, recommending an answer for each and explaining the trade-off, until every architectural branch is resolved.
...
- Trace every module and data-model decision back to the AC/US it serves; this is a spec _of the business docs_, not free invention. Where the business docs are silent, raise it during the grill rather than guessing.
