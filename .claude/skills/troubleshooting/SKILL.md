...
- **Observable surface.** What does a failure _look like_? Error codes/messages the system emits, health-check endpoints, where logs live, what metrics/alerts exist. Symptoms must be described as the operator actually sees them.
...
- **Diagnostic commands must be real and runnable** for this stack — real service names, log paths, health URLs, env vars. The _cause_ may be hypothesized, but the command to check it must work.
