...
Produce a **list of questions** the technical team should put to the BA / PO. Do not silently answer them yourself — these are questions _for the business_. Where a question can be answered by exploring the codebase, explore it first and fold the finding into the question (e.g. "The `orders` table has no `cancelled_at` column today — do you expect a soft-cancel or a hard delete?").
...
- These are _proposals for the BA/PO_, not edits you apply. Only write the changes into the source docs if the user explicitly asks.
...
_Can this story be built, tested, and shipped on its own?_
...
_Is the intent clear enough to discuss trade-offs, without over-specifying the solution?_
...
_Does this deliver observable value, and can we tell once it's live?_

- What does success look like in production — and what metric/analytics confirm it?
- What's the cost of _not_ doing it, and does the technical approach actually move that needle?
- Observability: what logs, metrics, or alerts must exist for support to operate and prove value?

## E — Estimable

_Do we know enough to size it with confidence?_
...
- What's the riskiest unknown — and can we spike it _before_ committing a number?

## S — Small

_Does it fit comfortably in one sprint?_
...
_Can every acceptance criterion be verified?_
